...
async def admin_main(request: Request):
    ...
    """
    user_session = verify_session(request)
    if not user_session:
...
async def admin_dashboard(request: Request):
    ...
    try:
        ...
        unread_fb_res = supabase.table("feedbacks").select("id", count="exact").eq("is_processed", False).execute()
        recent_fb_res = supabase.table("feedbacks").select("*").order("created_at", desc=True).limit(5).execute()
        ...
        model_map = {
            m["id"]: {
                "display_name": f"{m.get('brand', '')} - {m.get('model', '')}".strip(" -") or "Chưa đặt tên", 
                "count": 0
            } 
        ...
        category_stats = [{"name": v["display_name"], "count": v["count"]} for v in model_map.values()]
        if other_guides_count > 0:
            category_stats.append({"name": "Tài liệu chung / Khác", "count": other_guides_count})
        ...
        logger.error(f"Lỗi khi tải dữ liệu Dashboard Thống kê: {e}")
        return templates.TemplateResponse(
            "admin_dashboard.html", 
            {"request": request, "user": user_session}
        )
...
async def mark_feedback_processed(feedback_id: int, request: Request):
    ...
    try:
        supabase.table("feedbacks").update({"is_processed": True}).eq("id", feedback_id).execute()
        return JSONResponse(content={"success": True, "message": "Đã cập nhật trạng thái"})
    except Exception as e:
        logger.error(f"Lỗi khi cập nhật feedback #{feedback_id}: {e}")
        raise HTTPException(status_code=500, detail="Không thể cập nhật trạng thái feedback")